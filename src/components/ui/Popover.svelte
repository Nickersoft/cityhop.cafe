...
<Popover.Root {...props} bind:open>
	...
	<Popover.Portal>
		<Popover.Content
			class={[
				'from-background/50 to-background/95 ring-background/75',
				...
				'z-30 h-[max(75vh,500px)] w-[max(60vw,800px)] overflow-hidden rounded-2xl border bg-linear-to-br ring-1 shadow-xl shadow-black/20 backdrop-blur-md'
			]}
			sideOffset={8}
		>
			{@render content()}
